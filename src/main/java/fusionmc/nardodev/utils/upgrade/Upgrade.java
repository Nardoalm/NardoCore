package fusionmc.nardodev.utils.upgrade;
...
public class Upgrade {
   ...
   public static void setupUpgrades() {
      ...
      new Upgrade("§6MVP", "role.vip", 5000L, "35:1 : 1", "lp user {player} parent set mvp", "role.mvp");
      new Upgrade("§bMVP§6+", "role.mvp", 10000L, "35:3 : 1", "lp user {player} parent set mvpplus", "role.mvpplus");
   }
   ...
   public void onBuy(Profile profile) {
      ...
      if (QUEUE_VIPS.containsKey(player.getName()) && ((Upgrade)QUEUE_VIPS.get(player.getName())).equals(this)) {
         try {
            CashManager.addCash(player.getName(), this.cash);
         } catch (CashException var4) {
         }
   ...
   public void dispatchPack(Player player) {
      ...
      BaseComponent[] var3 = TextComponent.fromLegacyText("§a§lYAY! §aSua compra foi aprovada!\n \n§7Agora você possui o VIP " + StringUtils.formatColors(this.name) + "\n \n§7Caso você precise de\n§7ajuda entre em\n§7contato, através de\n§7nosso suporte\n§7clicando ");
      int var4 = var3.length;

      for(int var5 = 0; var5 < var4; ++var5) {
         BaseComponent baseComponent = var3[var5];
         component.addExtra(baseComponent);
      }

      TextComponent urlDiscord = new TextComponent("§0§laqui§7!");
      urlDiscord.setClickEvent(new ClickEvent(Action.OPEN_URL, "https://redehyzen.com.br/discord"));
      ...
      Bukkit.getOnlinePlayers().forEach((toSend) -> {
         NMS.sendTitle(toSend, StringUtils.getFirstColor(this.name) + player.getName(), "§ftornou-se " + StringUtils.formatColors(this.name));
      });
      ItemStack book = new ItemStack(Material.WRITTEN_BOOK);
      BookMeta meta = (BookMeta)book.getItemMeta();
      meta.setAuthor("FusionMC");
