package fusionmc.nardodev.utils.menus;
...
public class MenuUpgrades<T extends Upgrade> extends PagedPlayerMenu {
   ...
   public MenuUpgrades(Profile profile) {
      ...
      Iterator var4 = upgrades.iterator();

      while(var4.hasNext()) {
         Upgrade upgrade = (Upgrade)var4.next();
            ...
            if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getCurrentInventory())) {
               ...
               if (item != null && item.getType() != Material.AIR) {
                  if (evt.getSlot() == this.previousPage) {
                     ...
                     Upgrade upgrade = (Upgrade)this.cosmetics.get(item);
                     if (upgrade != null) {
                        if (!upgrade.has(this.player) && upgrade.canBuy(this.player) && profile.getStats("ApexServicesProfile", new String[]{"cash"}) >= upgrade.getCash()) {
                           EnumSound.CLICK.play(this.player, 0.5F, 2.0F);
