...
public class MenuBuyUpgrade<T extends Upgrade> extends PlayerMenu {
   ...
   public MenuBuyUpgrade(Profile profile, String name, T upgrade) {
      ...
      this.setItem(12, BukkitUtils.deserializeItemStack("DIAMOND : 1 : nome>&aConfirmar : desc>&7Comprar \"" + name + "\"\n&7por &b" + StringUtils.formatNumber(upgrade.getCash()) + " Cash&7."));
         ...
         if (evt.getWhoClicked().equals(this.player)) {
            ...
            if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getInventory())) {
               ...
               if (item != null && item.getType() != Material.AIR) {
                  if (evt.getSlot() == 12) {
                     if (profile.getStats("ApexServicesProfile", new String[]{"cash"}) < this.upgrade.getCash()) {
