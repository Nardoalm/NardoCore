   ...
   public void perform(CommandSender sender, String[] args) {
      if (sender.hasPermission("hyzenutils.cmd.staffchat")) {
         ...
         ProxyServer.getInstance().getPlayers().stream().filter((player) -> {
            ...
            player.sendMessage(TextComponent.fromLegacyText("§d§l[Staff] §7[" + finalServer + "] " + (sender instanceof ProxiedPlayer ? Role.getPlayerRole(finalPlayerr).getPrefix() + finalPlayerr.getName() : "§6[Master] CONSOLE") + "§f: " + StringUtils.formatColors(joined)));
         });
      } else {
