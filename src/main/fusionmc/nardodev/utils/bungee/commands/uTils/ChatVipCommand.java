   ...
   public void perform(CommandSender sender, String[] args) {
      if (sender.hasPermission("hyzenutils.cmd.chatvip")) {
         ...
         ProxyServer.getInstance().getPlayers().stream().filter((player) -> {
            ...
            player.sendMessage(TextComponent.fromLegacyText("§6§l[Chat Vip] §7[" + server + "] " + (sender instanceof ProxiedPlayer ? Role.getPlayerRole(playerr).getPrefix() + playerr.getName() : "§4[Supremo] CONSOLE") + "§f: " + StringUtils.formatColors(joined)));
         });
      } else {
