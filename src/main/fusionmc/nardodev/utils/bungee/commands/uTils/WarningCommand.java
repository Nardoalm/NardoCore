   ...
   public void perform(CommandSender sender, String[] args) {
      if (sender.hasPermission("hyzenutils.cmd.warning")) {
         ...
         ProxyServer.getInstance().getPlayers().forEach((player) -> {
            ...
            player.sendMessage(TextComponent.fromLegacyText("§r" + (sender instanceof ProxiedPlayer ? Role.getPlayerRole(playerr).getPrefix() + playerr.getName() : "§6[Master] CONSOLE") + "§f: " + join));
            player.sendMessage(TextComponent.fromLegacyText(""));
         });
      } else {
