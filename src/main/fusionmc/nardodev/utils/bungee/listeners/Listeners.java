...
public class Listeners implements Listener {
   ...
   public void onProxyPing(ProxyPingEvent evt) {
      ...
      if (config.getBoolean("version_change_enabled")) {
         pingPlayers.setSample(new PlayerInfo[]{new PlayerInfo(StringUtils.formatColors(config.getString("version_name")), UUID.randomUUID())});
   ...
   public void onChat(ChatEvent evt) {
      if (evt.getSender() instanceof ProxiedPlayer) {
         if (evt.isCommand()) {
            ...
            if (((List)Settings.blocked_commands$list.stream().map(String::toLowerCase).collect(Collectors.toList())).contains(command.toLowerCase())) {
               ...
               ProxyServer.getInstance().getPlayers().stream().filter((pp) -> {
                  return pp.hasPermission("hyzenutils.exe.notification");
               }).forEach((pp) -> {
                  pp.sendMessage(TextComponent.fromLegacyText(StringUtils.formatColors(Settings.blocked_commands$message_staff.replace("{player}", player.getName()).replace("{command}", "/" + command))));
               });
               evt.setCancelled(true);
            }
         }
